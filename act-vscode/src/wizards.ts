...
export async function promptAsync(repos: any[]): Promise<string | null> {
  const { value } = await prompts({
    type: 'select',
    name: 'value',
    message: 'Como você gostaria de começar',
    choices: [
      { title: 'Fonte de usuário mínimo', value: 'default' },
      {
        title: `Assistentes de ${terminalLink(
          'actools-vscode/wizards',
          'https://bitbucket.org/brodao/actools-vscode/src/wizards'
        )}`,
        value: 'wizards',
      },
      {
        title: `Assistentes de ${terminalLink('localmente (userhome)', 'file://user')}`,
        value: 'userhome',
      },
    ],
  });
  ...
  if (value === 'example') {
    ...
    try {
      ...
      console.log();
      console.log('Falha ao buscar a lista de assistente com o seguinte erro:');
      console.error(error);
      console.log();
      console.log('Mudando para o assistente inicial padrão');
      console.log();
    }
    ...
    if (examplesJSON) {
      ...
      if (!nameRes.wizardName) {
        console.log();
        console.log('Especifique um assistente ou use o assistente inicial padrão.');
        process.exit(1);
      }

      return nameRes.wizardName.trim();
    }
  }
...
export async function resolveWizardArgAsync(
  targetFolder: string,
  oraInstance: any,
  wizard: string,
  wizardPath?: string
  ...
  if (wizard) {
    ...
    if (repoUrl) {
      if (repoUrl.origin !== 'https://github.com' && repoUrl.origin !== 'https://bitbucket.org') {
        oraInstance.fail(
      ...
      repoInfo = await getRepoInfo(repoUrl, wizardPath);

      if (!repoInfo) {
        oraInstance.fail(
          `Found invalid GitHub URL: ${chalk.red(`"${wizard}"`)}. Please fix the URL and try again.`
        );
        process.exit(1);
      }
      ...
      const found = await hasExample(wizard);

      if (!found) {
        oraInstance.fail(
          `Não foi possível localizar o assistente chamado ${chalk.red(`"${wizard}"`)}.`
        );
        process.exit(1);
      }
    }
  ...
  if (repoInfo) {
    oraInstance.text = chalk.bold(
      `Downloading files from repo ${chalk.cyan(wizard)}. This might take a moment.`
    );

    await downloadAndExtractRepoAsync(targetFolder, repoInfo);
  } else {
    oraInstance.text = chalk.bold(
      `Downloading files for example ${chalk.cyan(wizard)}. This might take a moment.`
    );

    await downloadAndExtractExampleAsync(targetFolder, wizard);
  }

  await ensureProjectHasGitIgnore(targetFolder);
